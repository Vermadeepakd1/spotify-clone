...
async function getSongs(folder) {
    try {
        ...
        let a = await fetch(`${window.location.origin}/${currentfolder}/`);
        ...
        for (let i = 0; i < as.length; i++) {
            ...
            if (href && href.endsWith(".mp3")) {
                let fullUrl = new URL(href, `${window.location.origin}/${currentfolder}/`).href;
...
const playmusic = (songname, pause = false) => {
    ...
    currentSong.src = `${window.location.origin}/${currentfolder}/` + songname;
    if (!pause) {
        currentSong.play();
